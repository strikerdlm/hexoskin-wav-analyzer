...
import numpy as np
import pandas as pd
# BEGIN: guarded plotly import to allow module import without plotly installed
try:
	import plotly.graph_objects as go
	from plotly.subplots import make_subplots
	import plotly.offline as pyo
	_PLOTLY_AVAILABLE = True
except Exception:  # plotly not installed or misconfigured
	_PLOTLY_AVAILABLE = False
	go = None  # type: ignore
	def make_subplots(*args, **kwargs):  # type: ignore
		raise ImportError("Plotly is not installed. Please install plotly to use visualization features.")
	class _Pyo:
		def init_notebook_mode(self, **kwargs):
			return None
	pyo = _Pyo()
# END: guarded plotly import
...
class InteractivePlotter:
	...
	def __init__(self, 
		...
		# Set up plotly configuration for full-screen responsive plots (no-op if plotly missing)
		try:
	...
	def create_poincare_plot(self,
                           ...
                           show_statistics: bool = False) -> go.Figure:
		"""
	...
	def create_psd_plot(self,
                       ...
                       show_band_powers: bool = False) -> go.Figure:
		"""
	...
	def create_correlation_heatmap(self,
		...
		try:
			...
			fig = go.Figure(data=go.Heatmap(
				...
				hovertemplate='%{text}<extra></extra>',
				                colorbar=dict(
                    title=dict(text="Correlation Coefficient"),
                    x=1.02
                )
			))
    ...
    def create_time_series_plot(self,
		...
		try:
			if values is None or len(values) == 0:
				return self._create_error_figure("No data provided for time series plot")
			...
			fig.add_trace(go.Scatter(
				...
				name=metric_name or 'Series',
				hovertemplate='%{x}<br>%{y:.1f}<extra></extra>'
			))
			# Add trend line if requested
			if show_trend and len(values) > 10:
				try:
					trend_coeffs = np.polyfit(np.arange(len(values)), values, 1)
					trend_line = np.polyval(trend_coeffs, np.arange(len(values)))
					fig.add_trace(go.Scatter(
						x=timestamps,
						y=trend_line,
						mode='lines',
						line=dict(color='red', width=2, dash='dash'),
						name=f'Trend (slope: {trend_coeffs[0]:.2f} per unit)',
						hovertemplate='Linear Trend<br>%{x}<br>%{y:.1f}<extra></extra>'
					))
				except Exception as e:
			...
			if show_variability_bands:
				...
				fig.add_trace(go.Scatter(
					...
					line=dict(color='rgba(128,128,128,0)', width=0),
					name='±1 SD Band',
					hovertemplate='Variability Band<br>%{x}<br>%{y:.1f}<extra></extra>'
				))
			# Update layout with responsive full-screen configuration
			layout_config = self._get_responsive_layout()
			layout_config.update({
	...
	def export_plot(self, fig: 'go.Figure', output_path: Path, format: str = 'html') -> bool:
		"""Compatibility wrapper used in tests to export figures."""
		try:
			if format.lower() == 'html':
				return self.export_html(fig, str(output_path), include_plotlyjs='cdn')
			# PNG or others not supported in headless env without kaleido; return False gracefully
			return False
		except Exception:
			return False